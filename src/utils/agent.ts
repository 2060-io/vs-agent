...
import { createServiceAgent, ServiceAgent } from './ServiceAgent'
...
export const setupAgent = async ({
  ...
  if (!currentUserProfile.displayName) {
    ...
    const displayPicture = imageUrl ? { links: [imageUrl],
      mimeType: 'image/png' } : undefined
  ...
  if (publicDid) {
    ...
    agent.events.on(ConnectionEventTypes.ConnectionStateChanged, async (data: ConnectionStateChangedEvent) => {
      ...
      if (oob?.outOfBandInvitation.id === publicDid && data.payload.connectionRecord.state === DidExchangeState.RequestReceived) {
...
export async function createInvitation(agent: ServiceAgent) {
  ...
  return { url: outOfBandInvitation.toUrl({ domain: process.env.AGENT_INVITATION_BASE_URL ?? 'https://2060.io/i' }) }
}